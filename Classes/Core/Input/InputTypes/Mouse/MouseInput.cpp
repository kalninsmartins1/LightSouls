...
MouseInput::MouseInput()
	...
	, m_lastMousePos(Vector2::GetZero())
{
	if(!Init())
	{
...
void MouseInput::UpdateMouseAxis(const Vector2& newPos)
{
	...
	for (auto& axis : m_mouseAxis)
	{
		...
		{
			mouseAxis.curValue = GetPosInTargetRange(m_xAxisRange, targetRange, posDiff.GetX());
		}
		else if (curType == MouseAxisType::Y)
		{
			mouseAxis.curValue = GetPosInTargetRange(m_yAxisRange, targetRange, posDiff.GetY());
		}
	}
...
void MouseInput::OnMouseMoved(cc::EventMouse* eventData)
{
	...
	{	
		auto pos = eventData->getLocationInView();
		UpdateMouseAxis(Vector2(pos.x, pos.y));
	}
}
